// Hardcoded values for the season and plant type
let season = prompt("Please enter any season").toLowerCase()
let plantType = prompt("Please enter anpm plant type").toLowerCase()

// Variable to hold gardening advice
let advice = "";

//FUNCTION TO DETERMINE ADVICE BASED ON THE SEASON
function seasonAdvice(season) {
    if (season === "summer") {
        advice += "Water your plants regularly and provide some shade.\n";
    } else if (season === "winter") {
        advice += "Protect your plants from frost with covers.\n";
    } else {
        advice += "No advice for this season.\n";
    }

}

//FUNCTION TO DETERMINE ADVICE BASED ON TH PLANT TYPE
function plantAdvice(plantType) {
    if (plantType === "flower") {
        advice += "Use fertiliser to encourage blooms.";
    } else if (plantType === "vegetable") {
        advice += "Keep an eye out for pests!";
    } else {
        advice += "No advice for this type of plant.";
    }

}

seasonAdvice(season);
plantAdvice(plantType);

// Log the generated advice to the console
console.log(advice);

// TODO: Examples of possible features to add:
// - Add detailed comments explaining each block of code.
// - Refactor the code into functions for better readability and modularity.
// - Store advice in an object for multiple plants and seasons.
// - Suggest plants that thrive in the given season.